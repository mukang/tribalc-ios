...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        UILabel *payMethodLab = [TCComponent createLabelWithFrame:CGRectMake(20, 0, TCScreenWidth - 40, 40) AndFontSize:12 AndTitle:@"支付方式"];
        [self addSubview:payMethodLab];
        
        UIView *topLineView = [TCComponent createGrayLineWithFrame:CGRectMake(20, payMethodLab.y + payMethodLab.height - 0.5, TCScreenWidth - 40, 0.5)];
...
- (UIButton *)getPayViewWithImageName:(NSString *)imgName AndTitle:(NSString *)title AndFrame:(CGRect)frame{
    ...
    UIImageView *imgView = [[UIImageView alloc] initWithFrame:CGRectMake(30, frame.size.height / 2 - 18 / 2, 18, 18)];
    imgView.image = [UIImage imageNamed:imgName];
    [view addSubview:imgView];
    
    UILabel *titleLab = [TCComponent createLabelWithFrame:CGRectMake(imgView.x + imgView.width + 15, 0, 100, frame.size.height) AndFontSize:12 AndTitle:title];
    [view addSubview:titleLab];
    
    UIView *lineView = [TCComponent createGrayLineWithFrame:CGRectMake(20, frame.size.height - 0.5, TCScreenWidth - 40, 0.5)];
