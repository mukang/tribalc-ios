...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        UIView *alertView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, TCScreenWidth - 40 * 2, (TCScreenWidth - 40 * 1.7) / 2)];
        ...
        UIImageView *tagImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 16, 16)];
        tagImageView.image = [UIImage imageNamed:@"reserve_fail"];
        UILabel *titleLab = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, alertView.width, alertView.height - 50)];
        titleLab.text = @"是否确定取消订单";
        [titleLab sizeToFit];
        tagImageView.frame = CGRectMake(alertView.width / 2 - (tagImageView.width + titleLab.width) / 2, (alertView.height - 50) / 2 - 16 / 2, 16, 16);
        titleLab.frame = CGRectMake(tagImageView.x + tagImageView.width, tagImageView.y, titleLab.width, 16);
        [alertView addSubview:tagImageView];
        [alertView addSubview:titleLab];
        
        UIView *topLineView = [TCComponent createGrayLineWithFrame:CGRectMake(0, alertView.height - 50, alertView.width, 1)];
        [alertView addSubview:topLineView];
        
        UIView *centerLineView = [TCComponent createGrayLineWithFrame:CGRectMake(alertView.width / 2 - 0.5, alertView.height - 50 + 6, 1, 50 - 12)];
        [alertView addSubview:centerLineView];
        
        UIButton *cancelBtn = [self getButtonWithText:@"取消" AndFrame:CGRectMake(0, topLineView.y + topLineView.height, centerLineView.x, 50)];
        cancelBtn.tag = 0;
        [cancelBtn addTarget:self action:@selector(btnOnClick:) forControlEvents:UIControlEventTouchUpInside];
        [alertView addSubview:cancelBtn];
        
        UIButton *confirmBtn = [self getButtonWithText:@"确定" AndFrame:CGRectMake(cancelBtn.width + 1, cancelBtn.y, cancelBtn.width, cancelBtn.height)];
        confirmBtn.tag = 1;
